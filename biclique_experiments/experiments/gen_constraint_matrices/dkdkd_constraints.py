...
parser.add_argument('--bicfile', type=str, default='../../datafiles/Bicliques/DKDNextBicliques.txt', help='all bicliques file path')
...
for i in range(len(all_biclique_tuples)):
    ...
    for edge in List_of_Edges:
        ...
        if edge[0] in L0 and edge[1] in L1:
            M[edge_position][i]=1        
save_path=constraint_folder+bicfile_path[26:-13]+"NEWconstraints.npy"
Mx=np.array(M)
np.save(save_path, Mx)
print(Mx.shape)
new_save_path=constraint_folder+bicfile_path[26:-13]+"NEWconstraints.txt"
...
M_T=[list(i) for i in zip(*M)]
new_save_path=constraint_folder+bicfile_path[26:-13]+"NEWconstraintsTRANS.txt"
with open(new_save_path, "w") as f:
    for line in M_T:
        for i in range(len(line)):
            line[i]=str(line[i])
